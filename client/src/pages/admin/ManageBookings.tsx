    ...
    try {
      ...
      if (currentDateOnly.getTime() > checkInDateOnly.getTime()) {
        return {
          isValid: false,
          message:
            "Guest is arriving late. This was scheduled for " +
      ...
      if (isVenueBooking) {
        ...
        if (arrivalTimeParts.length >= 2) {
          ...
          if (
            currentTimeInMinutes < minCheckInTimeInMinutes &&
            arrivalTimeInMinutes >= minCheckInTimeInMinutes
          ) {
            return {
              isValid: false,
            ...
            >
              ...
              <div className="relative group">
                <motion.button
                  ...
                  className={`px-4 py-2 text-white rounded-lg cursor-pointer flex items-center justify-center gap-2 shadow-sm ${
                    canCheckIn
          ...
          {canManage && booking.status === "checked_in" && (
            ...
            >
              <motion.button
                whileTap={{ scale: 0.95 }}
                onClick={onCheckOut}
                className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg cursor-pointer transition-colors flex items-center justify-center gap-2 shadow-md duration-300"
              >
                <Check size={18} />
                Check Out
              </motion.button>
