...
const BookingCalendar = () => {
  ...
  const { data: bookingsData, isLoading: isLoadingBookings } = useQuery<{
    ...
    queryFn: () =>
      fetchRoomBookings(roomId || "", dateRange.startDate, dateRange.endDate),
    enabled: !!roomId,
  });
    ...
    if (checkInDate && checkOutDate && bookingsData?.data) {
      const hasOverlap = bookingsData.data.some((booking) => {
        ...
        const hasDateOverlap =
          checkInDate < existingCheckOut && existingCheckIn < checkOutDate;
  ...
  useEffect(() => {
    if (roomId) {
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: () =>
          fetchRoomBookings(roomId, prefetchStartDate, prefetchEndDate),
      });
    }
      ...
      if (isCheckout) {
        ...
        if (booking && booking.status) {
          ...
          if (["checked_in", "reserved"].includes(status)) {
            const matchingBooking = bookingsData?.data.find(
  ...
  const getDateCellClass = (date: Date) => {
    ...
    const isHovered =
      !isDateUnavailable(date, isCheckout) &&
      hoveredDate &&
    ...
    if (
      ...
      const matchingBooking = bookingsData?.data.find(
        (b) => bookingsByDate[format(date, "yyyy-MM-dd")]?.bookingId === b.id
      );
  ...
  const handleProceed = () => {
    if (
      ...
      navigate(
        `/confirm-booking?roomId=${roomId}&arrival=${format(
          checkInDate,
          "yyyy-MM-dd"
        )}&departure=${format(
          checkOutDate,
            ...
            {arrivalParam && departureParam ? (
              <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                <p className="font-medium text-blue-800">
                  Dates pre-selected:{" "}
                  {checkInDate && checkOutDate
                    ? `${format(checkInDate, "MMMM dd")} to ${format(
                        checkOutDate,
                        "MMMM dd, yyyy"
