...
const BookingDetailsModal: FC<BookingDetailProps> = ({
  booking,
  onClose,
  onConfirm,
  onReject,
  ...
  const isNoShowEligible = (): boolean => {
    ...
    const eligible =
      todayOnly.getTime() === checkInOnly.getTime() &&
      currentDate.getTime() > cutOff.getTime();
    ...
    try {
      ...
      if (currentDateOnly.getTime() <= checkInDateOnly.getTime()) {
        return {
          isValid: false,
          message:
            "Check-in is not available yet. Guest is arriving earlier than the scheduled date.",
        };
      }
      ...
      if (isVenueBooking) {
        ...
        if (currentDate >= venueCheckOutTime) {
          ...
          return {
            isValid: false,
            message:
              "Venue checkout is only allowed on or after the scheduled check-out date.",
          };
        }
      } else {
        ...
        if (currentDateOnly >= checkOutDateOnly) {
          ...
          return {
            isValid: false,
            message:
              "Room checkout is only allowed on or after the scheduled check-out date.",
          };
        }
      }
    } catch (error) {
            ...
            >
              <div className="relative group">
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-lg flex items-center justify-center gap-2 shadow-sm ${
                    canMarkNoShow
              ...
              <div className="relative group">
                <motion.button
                  ...
                  onClick={() =>
                    onCheckIn && canCheckIn && onCheckIn(currentPayment)
                  }
                  className={`px-4 py-2 text-white rounded-lg flex items-center justify-center gap-2 shadow-sm ${
                    canCheckIn
            ...
            >
              <div className="relative group">
                <motion.button
                  ...
                  className={`px-6 py-3 text-white rounded-lg transition-colors flex items-center justify-center gap-2 shadow-md duration-300 ${
                    canCheckOut
